...
def create_category(data: ProductCategoryCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
def list_products(skip: int = 0, limit: int = 100, search: str = "", category_id: int | None = None, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
